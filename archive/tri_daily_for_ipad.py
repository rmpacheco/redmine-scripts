...
while nextOffset < total_count:
    ...
    for x in xrange(0, setSize):
        ...
        if issue.estimated_sp <= 0 and issue.status == 1: # status 1 => new
            print ("Warning: Story %d doesn't have a story point estimate" % (
                issue.id))
...
for i in issues:
    ...
    if total_hours_for_issue > 0:
        ...
        if i.worked_sp > 0:
            print "Warning: Story %d claims progress but has 0 spent time recorded" % (
                i.id)
...
if total_projected_sp >= total_sp:
    print (projection_msg % (total_projected_sp))
else:
    print (projection_msg % (total_projected_sp))
print ""
